...
/**
 * TeaConfig contains all the configuration for the system. This class should not be called directly.
 *
 * <p>Framework settings are all in capital letters. Settings can be accessed via <code>Tea::conf()->SOME_SETTINGS</code></p>
 *
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaConfig.php 1000 2009-07-7 18:27:22
 * @package Tea.app
 * @since 1.0
 */
class TeaConfig{
    ...
    /**
     * Path to the location of Tea framework directory. eg. /var/lib/Teaframework/
     * @var string 
     */
    public $BASE_PATH;
    ...
    /**
     * URL of your app. eg. http://localhost/Tea/
     * @var string 
     */
    public $APP_URL;

    /**
     * Please define SUBFOLDER if your app is not in the root directory of the domain. eg. http://localhost/Tea , you should set '/Tea/'
     * @var string 
     */
    public $SUBFOLDER;
     ...
     * Debug tool can be viewed if <code>Tea::logger()->showDebugger()</code> is called
     * @var bool 
     */
	 ...
	 * Options are: TeaView and TeaBasicView
     * @var string
	 */
    ...
    /**
     * List of template tags to be used globally in TeaView template engine ( global function names and PHP functions )
     * @var array
     */
     ...
     * class MyController extends \TeaController {
     *     .....
     * } ?>
     *
     ...
     * <code>
     * //Store
     * Tea::conf()->add('latestItem', $item);
     *
     * //Retrieve
     * echo Tea::conf()->latestItem;
     * </code>
     *
...
/**
 * TeaWebApp is the global context that processed user's requests.
 *
 * <p>It manages the controllers in MVC pattern, handling URI requests 404 not found, redirection, etc.</p>
 *
 * <p>This class is tightly coupled with TeaUriRouter.</p>
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaWebApp.php 1000 2009-07-7 18:27:22
 * @package Tea.app
 * @since 1.0
 */
class TeaWebApp{
    ...
    public function routeTo(){
        Tea::loadCore('uri/TeaUriRouter');
        $router = new TeaUriRouter;
        $routeRs = $router->execute($this->route,Tea::conf()->SUBFOLDER);
        ...
        if($routeRs[0]!==null && $routeRs[1]!==null){
            ...
            if($routeRs[0][0]!=='['){
                if(strpos($routeRs[0], '\\')!==false){
                    ...
                    $nsClassFile = explode(Tea::conf()->APP_NAMESPACE_ID.'/', $nsClassFile, 2);
                ...
                if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===true){
                    require_once Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER_ORI . 'module/'. $moduleName .'/controller/'.$moduleParts[1].'.php';                    
                }else{
                    require_once Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'module/'. $moduleName .'/controller/'.$moduleParts[1].'.php';                    
                    Tea::conf()->PROTECTED_FOLDER_ORI = Tea::conf()->PROTECTED_FOLDER;
                }
                
                //set class name
                $routeRs[0] = $moduleParts[1];
                Tea::conf()->PROTECTED_FOLDER = Tea::conf()->PROTECTED_FOLDER_ORI . 'module/'.$moduleName.'/';
            }
        ...
        //if auto route is on, then auto search Controller->method if route not defined by user
        else if(Tea::conf()->AUTOROUTE){

            list($controller_name, $method_name, $method_name_ori, $params, $moduleName )= $router->auto_connect(Tea::conf()->SUBFOLDER, (isset($this->route['autoroute_alias'])===true)?$this->route['autoroute_alias']:null );
            ...
            $controller_file = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "controller/{$controller_name}.php";
            ...
            if(file_exists($controller_file)){
                ...
                if($methodsArray===null && isset(Tea::conf()->APP_NAMESPACE_ID)===true){
                    if(isset($moduleName)===true){
                        $controller_name = Tea::conf()->APP_NAMESPACE_ID . '\\module\\'. $moduleName .'\\controller\\' . $controller_name;                        
                    }else{
                        $controller_name = Tea::conf()->APP_NAMESPACE_ID . '\\controller\\' . $controller_name;
                    }
    				$methodsArray = get_class_methods($controller_name);   
                }
                ...
                if($methodsArray===null){
                    if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===true)
                        Tea::conf()->PROTECTED_FOLDER = Tea::conf()->PROTECTED_FOLDER_ORI;
					$this->throwHeader(404);
					return;                    
                }
            }
            else if(isset($moduleName)===true && isset(Tea::conf()->APP_NAMESPACE_ID)===true){
                if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===true)
                    Tea::conf()->PROTECTED_FOLDER = Tea::conf()->PROTECTED_FOLDER_ORI;                
                
                $controller_file = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . '/controller/'.$moduleName.'/'.$controller_name .'.php';                 
                
                if(file_exists($controller_file)===false){
					$this->throwHeader(404);
					return;                    
                }                
                $controller_name = Tea::conf()->APP_NAMESPACE_ID .'\\controller\\'.$moduleName.'\\'.$controller_name;                
    ...
    public function reroute($routeuri, $is404=false){
        ...
        if(Tea::conf()->SUBFOLDER!='/')
            $_SERVER['REQUEST_URI'] = substr(Tea::conf()->SUBFOLDER, 0, strlen(Tea::conf()->SUBFOLDER)-1) . $routeuri;
        else
            $_SERVER['REQUEST_URI'] = $routeuri;

        if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===true){
            Tea::conf()->PROTECTED_FOLDER = Tea::conf()->PROTECTED_FOLDER_ORI;
            unset( Tea::conf()->PROTECTED_FOLDER_ORI );
        }
     ...
     * $data['top'] = Tea::app()->module('/top/banner');
     * </code>
     *
     * <p>Based on Controller name and Action method:</p>
     * <code>
     * Tea::app()->module('MyController', 'renderTop');
     *
     * # If controller is in sub folder
     * Tea::app()->module('folder/MyController', 'renderTop');
     *
     * # Passed in parameter if controller is using $this->param['var']
     * Tea::app()->module('MyController', 'renderTop', array('nav'=>'banner'));
     * </code>
     *
     * <p>If class name is different from controller filename:</p>
     * <code>
     * # filename is index.php, class name is Admin
     * Tea::app()->module(array('index', 'Admin'), 'renderTop');
     *
     * # in a sub folder
     * Tea::app()->module(array('admin/index', 'Admin'), 'renderTop');
     *
     * # with parameters
     * Tea::app()->module(array('admin/index', 'Admin'), 'renderTop', array('nav'=>'banner'));
     * </code>
     *
    ...
    public function module($moduleUri, $action=null, $params=null){
        if($moduleUri[0]=='/'){
            if(Tea::conf()->SUBFOLDER!='/')
                $_SERVER['REQUEST_URI'] = substr(Tea::conf()->SUBFOLDER, 0, strlen(Tea::conf()->SUBFOLDER)-1) . $moduleUri;
            else
                $_SERVER['REQUEST_URI'] = $moduleUri;

            $tmp = Tea::conf()->PROTECTED_FOLDER;
            if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===true){
                Tea::conf()->PROTECTED_FOLDER = Tea::conf()->PROTECTED_FOLDER_ORI;
                $tmpOri = Tea::conf()->PROTECTED_FOLDER_ORI;
            }
            ...
            Tea::conf()->PROTECTED_FOLDER = $tmp;
            
            if(isset($tmpOri)===true)
                Tea::conf()->PROTECTED_FOLDER_ORI = $tmpOri;
            
            return $data;
        }
        //if Controller name passed in:  Tea::app()->module('admin/SomeController', 'login',  array('nav'=>'home'));
            ...
            require_once Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "controller/$moduleUri.php";
            $controller = new $controller_name;
            $controller->params = $params;
            ...
            if($controller->autorender===true){
                Tea::conf()->AUTO_VIEW_RENDER_PATH = array(strtolower(substr($controller_name, 0, -10)), strtolower(preg_replace('/(?<=\\w)(?=[A-Z])/','-$1', $action)));
            }
        ...
        //eg. Tea::app()->module(array('admin/Admin', 'AdminController'), 'login',  array('nav'=>'home'));
        else{
            require_once Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "controller/{$moduleUri[0]}.php";
    ...
    /**
     * Advanced version of TeaWebApp::module(). Process a module from the main application or other modules.
     *
    ...
    public function getModule($moduleName, $moduleUri, $action=null, $params=null){
        if(empty($moduleName)===false){
            if(isset(Tea::conf()->PROTECTED_FOLDER_ORI)===false){
                Tea::conf()->PROTECTED_FOLDER_ORI = $tmp = Tea::conf()->PROTECTED_FOLDER;
                Tea::conf()->PROTECTED_FOLDER = $tmp . 'module/'.$moduleName.'/';
        ...
        if($code!=null){
            if(is_int($code)){
                if($code===404){
                    ...
                    header('HTTP/1.1 404 Not Found');
                    if(!empty(Tea::conf()->ERROR_404_DOCUMENT)){
                        include Tea::conf()->SITE_PATH . Tea::conf()->ERROR_404_DOCUMENT;
                    }
                    ...
                    elseif(!empty(Tea::conf()->ERROR_404_ROUTE)){
						$this->reroute(Tea::conf()->ERROR_404_ROUTE, true);
                    }
                    exit;
                }
                //if not 404, just send the header code
                else{
                    TeaUriRouter::redirect(null,true, $code);
                }
            }
            elseif(is_string($code)){
                //Controller return the redirect location, it sends 302 Found
                TeaUriRouter::redirect($code);
            }
            elseif(is_array($code)){
                ...
                elseif($code[1]===302){
                    TeaUriRouter::redirect($code[0],true, $code[1], array("HTTP/1.1 302 Moved Temporarily"));
                }
                //else redirect with the http status defined,eg. 307
                else{
                    TeaUriRouter::redirect($code[0],true, $code[1]);
                }
            }
        }
    }
    ...
    /**
     * To debug variables with Tea's diagnostic view
     * @param mixed $var The variable to view in diagnostics.
     */
    public function debug($var){
        throw new TeaDebugException($var);
    }

}

/**
 * TeaUriRouter class file.
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @link http://www.Tea.com/
 * @copyright Copyright &copy; 2009 Leng Sheng Hong
...
/**
 * TeaUriRouter parse the server request URI
 *
 ...
 * in both mod_php or FastCGI mode. Please refer to http://Tea.com/tutorial/setup to see how to used Tea with Cherokee web server</p>
 *
 ...
 * </p>
 *
 * <p><b>RESTful API</b>s are supported natively. You can mocked up <b>RESTful API</b>s easily with TeaUriRouter.
 ...
 * </code></p>
 *
 * <p>If you need to reverse generate URL based on route ID with TeaUrlBuilder in template view, please defined the id along with the routes
 * <code>
 ...
 * </code></p>
 *
 * <p>If you want a module to be publicly accessed (without using Tea::app()->getModule() ) , use [module name] ,   eg. /protected/module/forum/PostController.php
 * <code>
 ...
 * </p>
 *
 * <p>See http://Tea.com/doc/guide/uri-routing for information in configuring Routes</p>
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: Tea.php 1000 2009-07-7 18:27:22
 * @package Tea.uri
...
class TeaUriRouter{
     ...
     * @param string $subfolder Relative path of the sub directory where the app is located. eg. http://localhost/Tea, the value should be '/Tea/'
    ...
    public function execute($routeArr,$subfolder='/'){
        ...
        if(isset($route['auth'])===true){
            ...
            Tea::loadCore('auth/TeaDigestAuth');
            TeaDigestAuth::http_auth($route['authName'],$route['auth'], $route['authFail'], $route['authFailURL']);
        }
     ...
     * <p>To use TeaUriRouter without index.php, add the following code to your .htaccess file if Apache mod_rewrite is enabled.</p>
     * <code>
     * RewriteEngine On
     *
	...
	private function connect($routes, $subfolder) {
		...
		if ($skipNormalRoutes===false) {
			...
			if (isset($possibleRoutes[$requestedUri])===true) {
				...
				if (false === strpos($requestedUri, ':')) {
					...
					Tea::conf()->AUTO_VIEW_RENDER_PATH = $requestedUri;
		...
		if ($skipNormalRoutes===false) {
			foreach($possibleRoutes as $routeKey=>$routeData) {
				...
				Tea::conf()->AUTO_VIEW_RENDER_PATH = $routeKey;
    ...
    public function auto_connect($subfolder='/', $autoroute_alias=null){
        ...
        $module = null;
        if(isset(Tea::conf()->MODULES)===true && in_array($uri[0], Tea::conf()->MODULES)===true){
            $module = $uri[0];
            array_shift($uri);
        }
		...
		Tea::conf()->AUTO_VIEW_RENDER_PATH = array($controller_name);
        ...
        if(empty($uri[1])===false){
			...
			Tea::conf()->AUTO_VIEW_RENDER_PATH[] = $uri[1];
		}else{
            $method_name = $method_name_ori = 'index';
			Tea::conf()->AUTO_VIEW_RENDER_PATH[] = 'index';
		}
        ...
        if($autoroute_alias!==null){
            ...
            if(isset($autoroute_alias[$alias])===true){
                ...
                Tea::conf()->AUTO_VIEW_RENDER_PATH[0] = strtolower(preg_replace('/([A-Z])/', '-$1', substr($convertname, 0, -10)));
            }
            else{
                ...
                if( preg_match('/^('. $aliaskey .')\//', '/'.$uridecode.'/', $matchedKey) > 0){
                    ...
                    Tea::conf()->AUTO_VIEW_RENDER_PATH[1] = $method_name;
                }
            }
        }
 ...
 *
 * <p>Provides a few shorthand methods to access commonly used component during development. e.g. TeaLoader, TeaLog, TeaSqlMagic.</p>
 *
 ...
 *
 * <p>GET and POST variables can still be accessed via php $_GET and $_POST. They are not handled/process by Tea framework.</p>
 *
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaController.php 1000 2009-07-7 18:27:22
...
class TeaController {
    ...
    public function load(){
        if($this->_load==NULL){
            Tea::loadCore('uri/TeaLoader');
            $this->_load = new TeaLoader;
        }
    ...
    /**
     * Returns the database singleton, shorthand to Tea::db()
     * @return TeaSqlMagic
     */
    public function db(){
        return Tea::db();
    ...
    /**
     * Returns the Acl singleton, shorthand to Tea::acl()
     * @return TeaAcl
     */
    public function acl(){
        return Tea::acl();
    ...
    /**
     * Returns the cache singleton, shorthand to Tea::cache()
     * @return TeaFileCache|TeaFrontCache|TeaApcCache|TeaMemCache|TeaXCache|TeaEAcceleratorCache
     */
    public function cache($cacheType='file'){
        return Tea::cache($cacheType);
    }
     ...
     * @param bool $includeTagClass If true, TeaView will determine which Template tag class to include. Else, no files will be loaded
     * @return string|false The file name of the rendered output saved (html).
     */
     ...
     * @return TeaView|TeaViewBasic
     */
    public function view(){
        if($this->_view==NULL){
			$engine = Tea::conf()->TEMPLATE_ENGINE;
            Tea::loadCore('view/' . $engine);
	...
	public function viewRenderAutomation(){
		if(is_string(Tea::conf()->AUTO_VIEW_RENDER_PATH)){
			$path = Tea::conf()->AUTO_VIEW_RENDER_PATH;
			$path = str_replace(':', '@', substr($path, 1));
			$this->{$this->renderMethod}($path, $this->vdata);
		}else{
            if(isset(Tea::conf()->AUTO_VIEW_RENDER_PATH))
                $this->{$this->renderMethod}(strtolower(Tea::conf()->AUTO_VIEW_RENDER_PATH[0]) .'/'. strtolower(Tea::conf()->AUTO_VIEW_RENDER_PATH[1]), $this->vdata);
            else
	...
	public function  __call($name,  $arguments) {
		if ($name == 'renderLayout') {
			throw new Exception('renderLayout is no longer supported by TeaController. Please use $this->view()->renderLayout instead');
		}
	}
