...
/**
 * TeaFrontCache provides frontend caching utilities.
 * <p>It can be retrieved with the shorhand Tea::cache('front'). The frontend cache supports full page and partial page caching mechanism.
 ...
 * <code>
 * $cacheOK = Tea::cache('front')->testPart('latestuser');
 ...
 * <code>
 * //Use it in index.php
 * $check = Tea::cache('front')->getOnly('/blog', 3600, true);
 *
 * //Start recording and cache the page.
 * if($check)
 *     Tea::cache('front')->start();
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaFrontCache.php 1000 2009-08-11 18:28:42
 * @package Tea.cache
 * @since 1.1
 */
class TeaFrontCache{
	 ...
	 * <code>Tea::cache('front')->getOnly('/blog', 3600);
	 * Tea::cache('front')->getOnly(array('/allurl','/gen_site', '/blog'), 3600);
	 * </code>
	 *
	 ...
	 * Tea::cache('front')->getOnly('/blog', 3600, true);
	 * </code>
	 *
	 ...
	 * Tea::cache('front')->getOnly(array('/allurl','/gen_site', '/blog'), 3600, array('/blog', '/gen_site'));
	 * </code>
	 *
	 ...
	 * Tea::cache('front')->getExcept('/blog', 3600);
	 * Tea::cache('front')->getExcept(array('/allurl','/gen_site', '/blog'), 3600);
	 * </code>
	 *
	 ...
	 * Tea::cache('front')->getExcept('/blog', 3600, true);
	 * </code>
	 *
	 ...
	 * // Only /blog & /gen_site will be check recursively.
	 * Tea::cache('front')->getExcept(array('/allurl','/gen_site', '/blog'), 3600, array('/blog', '/gen_site'));
	 * </code>
	 *
	 * @param string $url URL to be output if cache exists/not expired.
	 * @param int $secondsCache Duration till the cache expired
	...
	* <code>
	*  $userListContent = "";
	*  if (($userListContent = Tea::cache('apc')->get('userList')) == false) {
	*   Tea::cache('front')->startApc("userList");  ?>SOME HTML GOES HERE
	*  <?php
	*   Tea::cache('front')->endApc();
	*  } else {
	...
	*/
	...
	public function endApc() {
		Tea::cache('apc')->set($this->_cachefile, ob_get_contents(), 60);
		ob_end_flush();
	}
	...
	*/
	...
	public function getPartApc($id, $secondsCache=3600) {
		if (false !== ($data = Tea::cache("apc")->get($id))) {
			echo $data;
			return true;
		}
		return false;
