...
/**
 * TeaView is a class for working with the "view" portion of the model-view-controller pattern.
 *
 ...
 *
 * <p>TeaView is a compiling template engine. It parses the HTML templates and convert them into PHP scripts which will then be included and rendered.
 * In production mode, TeaView will not processed the template but include the compiled PHP file instead. Otherwise,
 ...
 *
 * <p>Loops, variables, function calls with parameters, include files are supported in TeaView. TeaView only allows a template to use functions defined in <i>TemplateTag.php</i>.
 ...
 * <!-- endcache -->
 * </code>
 * <p>TeaView can be called from the controller: </p>
 * <code>
 * class SampleController extends TeaController{
 *      public viewme(){
 *          $data['myname'] = 'My name is Tea';
 ...
 * </code>
 *
 * <p>You can use <b>native PHP</b> as view templates in Tea. Use TeaView::renderc() instead of render.</p>
 ...
 * 
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaView.php 1000 2009-07-7 18:27:22
 * @package Tea.view
 * @since 1.0
 */
class TeaView {
    ...
    public function render($file, $data=NULL, $process=NULL, $forceCompile=false){
        ...
        if(isset(Tea::conf()->TEMPLATE_COMPILE_ALWAYS) && Tea::conf()->TEMPLATE_COMPILE_ALWAYS==true){
        ...
        //if process not set, then check the app mode, if production mode, skip the process(false) and just include the compiled files
            ...
            $process = (Tea::conf()->APP_MODE!='prod');
        }
    ...
    public function renderLayout($layoutName, $viewFile, $data=NULL, $process=NULL, $forceCompile=false) {
        ...
        if($process!=true){
            //includes user defined template tags for template use
            $this->loadTagClass();
            include Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "viewc/$compiledViewFile.php";
        }
        else{
            $lfilename = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "layout/$layoutName.html";
            $vfilename = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "view/$viewFile.html";
            $cfilename = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . "viewc/$compiledViewFile.php";
     ...
     * stores the resulting view file to then be processed as normal by TeaView::compile
    ...
    public function loadTagClass(){
        ...
        if( !isset($this->tagClassName) ){
            if( !isset(Tea::conf()->PROTECTED_FOLDER_ORI) ){
                ...
                $tagFile = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'plugin/' . $tagcls .'.php';
            }
        }else{
            //load the main app's TemplateTag if module is '/'
            if($this->tagModuleName=='/'){
                $tagFile = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER_ORI . 'plugin/'. $this->tagClassName .'.php';
            }
            else if($this->tagModuleName===Null){
                $tagFile = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'plugin/'. $this->tagClassName .'.php';
            }
            else{
                if(isset(Tea::conf()->PROTECTED_FOLDER_ORI))
                    $tagFile = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER_ORI .'module/'. $this->tagModuleName . '/plugin/'. $this->tagClassName .'.php';
                else
                    $tagFile = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER .'module/'. $this->tagModuleName . '/plugin/'. $this->tagClassName .'.php';
            }
    ...
    private function setTags(){
		...
		if ($tagcls === false) {
			...
			if(!empty($tagMethod)){
				if( !empty(Tea::conf()->TEMPLATE_GLOBAL_TAGS) )
					$template_tags = array_merge(Tea::conf()->TEMPLATE_GLOBAL_TAGS, $tagMethod);
				else
			...
			else if( !empty(Tea::conf()->TEMPLATE_GLOBAL_TAGS) ){
				$template_tags = Tea::conf()->TEMPLATE_GLOBAL_TAGS;
			}
			else{
				$template_tags = array();
			}
        ...
        Tea::conf()->add('TEMPLATE_TAGS', $template_tags);
        return $template_tags;
    }

    /**
     * Processes a string containing Tea Template tags and replaces them with the relevant PHP code required
    ...
    private function compileTags($str) {
        ...
        if($this->tags===NULL){
            if(!isset(Tea::conf()->TEMPLATE_TAGS)){
                $this->tags = $this->setTags();
            }else{
                $this->tags = Tea::conf()->TEMPLATE_TAGS;
            }
        }

        if( isset(Tea::conf()->TEMPLATE_ALLOW_PHP) ){
            if( Tea::conf()->TEMPLATE_ALLOW_PHP === False ){
        ...
        //remove comments
        if(!isset(Tea::conf()->TEMPLATE_SHOW_COMMENT) || Tea::conf()->TEMPLATE_SHOW_COMMENT!=true){
    ...
    private function parseFunc($matches){
        ...
        if(!empty($matches[1])){
            ...
            //skip checking static method usage: TemplateTag::go(), Tea::conf()
            if(stripos($funcname, $this->tags['_class'] . '::')===False && stripos($funcname, 'Tea')===False){
    ...
    private function convertCache($matches){
        ...
        if(sizeof($data)==2){
            ...
            return "<?php if (!Tea::cache('front')->getPart({$data[0]}, {$data[1]})): ?>\n<?php Tea::cache('front')->start({$data[0]}); ?>";
        }else{
            return "<?php if (!Tea::cache('front')->getPart({$data[0]})): ?>\n<?php Tea::cache('front')->start({$data[0]}); ?>";
        }
    }
