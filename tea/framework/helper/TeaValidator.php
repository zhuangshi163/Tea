 ...
 *
 * <p>TeaValidator can be used for form and Model data validation before saving/inserting/deleting a data.</p>
 *
 * <p>To use TeaValidator, you have to create an instance of it and defined the validation rules.
 ...
 * </code>
 *
 * <p>You can get the list of available validation rules by calling TeaValidator::getAvailableRules()</p>
 *
 * <p>To validate the data, create an instance of TeaValidator and call validate() in your Controller/Model.</p>
 * <code>
 * $v = new TeaValidator();
 *
 * # There are 3 different validation Mode.
 * //$v->checkMode = TeaValidator::CHECK_SKIP;
 * //$v->checkMode = TeaValidator::CHECK_ALL_ONE;
 * //$v->checkMode = TeaValidator::CHECK_ALL;
 *
 * //The $_POST or data pass in need to be an assoc array
 * //$data = array('username'=>'Tea', 'pwd'=>'12345');
 ...
 * If your Models are extending TeaModel or TeaSmartModel, you can validate the data by calling TeaModel::validate()</p>
 * <code>
 * $f = new Food;
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaValidator.php 1000 2009-08-30 11:37:22
 * @package Tea.helper
 * @since 1.2
 */
class TeaValidator {
        ...
        foreach($data as $k=>$v){
            ...
            foreach($cRule as $v2){
                if(is_array($v2)){
					...
					$vIsEmpty = ($this->requireMode == TeaValidator::REQ_MODE_NULL_EMPTY && ($v === null || $v === '') ||
								 $this->requireMode == TeaValidator::REQ_MODE_NULL_ONLY  && $v === null) ? true : false;
                    ...
                    if($err = call_user_func_array(array(&$this, 'test'.$v2[0]), $vv) ){
                        if($this->checkMode==TeaValidator::CHECK_ALL)
                            $errors[$k][$v2[0]] = $err;
                        else if($this->checkMode==TeaValidator::CHECK_SKIP && !$vIsEmpty && $v2[0]!='optional'){
                            return $err;
                        }else if($this->checkMode==TeaValidator::CHECK_ALL_ONE)
                            $errors[$k] = $err;
                    }
                    ...
                    if(sizeof($cRule)>1){
                        ...
                        if($err = call_user_func_array(array(&$this, 'test'.$cRule[0]), $vv) ){
                            if($this->checkMode==TeaValidator::CHECK_ALL || $this->checkMode==TeaValidator::CHECK_ALL_ONE)
                                $errors[$k] = $err;
                            else if($this->checkMode==TeaValidator::CHECK_SKIP){
                                return $err;
                            }
     ...
     * @param type $displayMethod Default error message display method. use: TeaValidator::REQ_MSG_UNDERSCORE_TO_SPACE, TeaValidator::REQ_MSG_CAMELCASE_TO_SPACE, TeaValidator::REQ_MSG_THIS, TeaValidator::REQ_MSG_FIELDNAME
     * @param type $suffix suffix for the error message. Default is ' field is required'
     */
    public function setRequiredFieldDefaults( $displayMethod = TeaValidator::REQ_MSG_UNDERSCORE_TO_SPACE, $suffix = ' field is required'){
    ...
    public function getRequiredFieldDefaultMsg($fieldname){
        if($this->requiredMsgDefaultMethod==TeaValidator::REQ_MSG_UNDERSCORE_TO_SPACE)
            return ucfirst(str_replace('_', ' ', $fieldname)) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==TeaValidator::REQ_MSG_THIS)
            return 'This ' . $this->requiredMsgDefaultSuffix;        
        
        if($this->requiredMsgDefaultMethod==TeaValidator::REQ_MSG_CAMELCASE_TO_SPACE)
            return ucfirst(strtolower(preg_replace('/([A-Z])/', ' $1', $fieldname))) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==TeaValidator::REQ_MSG_FIELDNAME)
            return $fieldname . $this->requiredMsgDefaultSuffix;
    }

    public function testOptional($value){}
    public function testRequired($value, $msg){
		if ($this->requireMode == TeaValidator::REQ_MODE_NULL_EMPTY && ($value === null || $value === '') ||
			$this->requireMode == TeaValidator::REQ_MODE_NULL_ONLY  && $value === null) {
    ...
    */
    public function testDbExist($value, $table, $field, $msg=null) {
        $result = Tea::db()->fetchRow("SELECT COUNT($field) AS count FROM " . $table . ' WHERE '.$field.' = ? LIMIT 1', array($value));
